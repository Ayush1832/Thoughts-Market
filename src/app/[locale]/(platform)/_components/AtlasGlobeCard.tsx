  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const [lat, lon] of LAND) {
        ...
        ctx.fillStyle = `rgba(30,190,255,${a * 0.52})`
        ctx.fill()
      }
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-1">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-2xs font-bold tracking-[1.8px] text-muted-foreground/70 uppercase">
            Atlas
            {' '}
      ...
      >
        {[
          ...
          <div key={s.label} className="relative">
            {i > 0 && (
              <span
                className="absolute top-1/2 left-0 h-8 w-px -translate-y-1/2"
                style={{ background: 'rgba(255,255,255,0.07)' }}
              />
            )}
            <p className={`text-lg leading-none font-bold ${s.danger ? 'text-red-400' : 'text-foreground'}`}>
              {s.val}
              {s.sub && <span className="text-[11px] font-normal text-muted-foreground/50">{s.sub}</span>}
            </p>
            <p className="mt-1 text-[8.5px] font-medium tracking-[0.7px] text-muted-foreground/35 uppercase">{s.label}</p>
          </div>
        ))}
      </div>

      {/* Threat list */}
      <div className="space-y-0 px-4 pb-1">
        {[
          ...
          <div
            key={t.name}
            className="flex items-center justify-between border-b border-white/5 py-[7px] last:border-0"
          >
            <div className="flex items-center gap-2.5">
              <span className="size-2 shrink-0 rounded-full" style={{ background: t.dot, boxShadow: `0 0 6px ${t.dot}88` }} />
      ...
      <div className="p-4 pt-3">
        <button
          className="
            w-full rounded-xl py-[10px] text-[13px] font-semibold transition-all duration-200
