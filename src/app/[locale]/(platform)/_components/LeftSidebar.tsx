...
function NavBadge({ variant, badge, count }: { variant?: string, badge?: string, count?: number }) {
  ...
  if (variant === 'reels') {
    return (
      <span className="rounded-sm bg-purple-500/20 px-1.5 py-0.5 text-2xs font-bold tracking-wider text-purple-400">
        {badge}
      </span>
    )
  }
...
export default function LeftSidebar() {
  ...
  return (
    <aside className="
      sticky top-0 hidden h-screen w-64 shrink-0 flex-col overflow-y-auto border-r border-border/50 bg-card
      lg:flex
      xl:w-72
    "
    >
